                ...
                using SqlConnection conn = new SqlConnection(CONNECTION_STRING);
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        ...
                        while (dr.Read())
                        {
                            ...
                            bans.Add(new Ban(Convert.ToInt32(dr[0]), Convert.ToString(dr[1]), user, admin, DateOnly.FromDateTime(Convert.ToDateTime(dr[5])), DateOnly.FromDateTime(Convert.ToDateTime(dr[6]))));
                        }
                    }
                }
                return bans.ToArray();
